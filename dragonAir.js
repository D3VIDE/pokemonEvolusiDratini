// =================================================================
// dragonAir.js (FIXED: FSM Logic for Random Wander)
// =================================================================
...
// =================================================================
// GEOMETRI DASAR (createDragonairSmoothBody, createSphere, createCone, createDragonairEarParaboloid)
...
function Dragonair(gl, programInfo) {
    ...
    this.moveSpeed = 5.0; 
    this.turnSpeed = 90.0;
    this.targetReachedThreshold = 1.0; 
    this.facingThreshold = 5.0;

    this.animationState = "IDLE_STATIC"; 
    this.stateTimer = 0.0;
    
    // [PERBAIKAN] Tambahkan radius untuk jalan acak & set target awal
...
// [PERBAIKAN] Logika FSM di-revisi
// =================================================================
Dragonair.prototype.update = function(now, groundY, elapsed) {
    ...
    switch (this.animationState) {
        ...
        case 'DYNAMIC_IDLE': // Ini adalah state "Pilih target baru"
            ...
            let dx_idle = this.targetFruitPosition[0] - this.position[0];
            let dz_idle = this.targetFruitPosition[1] - this.position[2];
            let dist_idle = Math.sqrt(dx_idle * dx_idle + dz_idle * dz_idle);
    
            // Jika sudah sampai (atau baru mulai), cari target baru
            if (dist_idle < this.targetReachedThreshold) { 
                let angle = Math.random() * 2 * Math.PI;
                let radius = this.idleRadius * (0.5 + Math.random() * 0.5); 
                
                let newTargetX, newTargetZ;
                let tries = 0;
                
                // Cari titik baru yang tidak diblokir (tidak di danau)
                do {
                    newTargetX = radius * Math.cos(angle);
                    newTargetZ = radius * Math.sin(angle);
                    angle += 0.5; 
                    tries++;
                } while (window.isPositionBlocked && window.isPositionBlocked(newTargetX, newTargetZ) && tries < 10);

                this.targetFruitPosition[0] = newTargetX;
                this.targetFruitPosition[1] = newTargetZ; 
                
                this.animationState = "DYNAMIC_START_WALK"; // <-- Ganti ke state jalan acak
            } else {
                // Jika dipanggil tapi belum sampai (misal: saat load)
                this.animationState = "DYNAMIC_START_WALK"; // <-- Ganti ke state jalan acak
            }
            waveTimeFactor = now * 0.008; 
            break;
        
        // [PERBAIKAN] State baru untuk berbelok (jalan acak)
        case "DYNAMIC_START_WALK":
            ...
            if (Math.abs(angleDiff_dyn) < this.facingThreshold || Math.abs(angleDiff_dyn) < turnAmount_dyn) {
                ...
                this.animationState = "DYNAMIC_WALK_TO_TARGET"; // Pindah ke fase maju
            } else {
                this.currentAngleY += Math.sign(angleDiff_dyn) * turnAmount_dyn;
            }
            break;

        // [PERBAIKAN] State baru untuk berjalan (jalan acak)
        case "DYNAMIC_WALK_TO_TARGET":
            waveTimeFactor = now * 0.15; // Liukan sangat cepat
            ...
            // [PERBAIKAN] Jika sampai, kembali ke DYNAMIC_IDLE, JANGAN MAKAN
            if (distToTarget_dyn_walk < this.targetReachedThreshold) {
                this.animationState = "DYNAMIC_IDLE"; // <-- Loop kembali
                this.stateTimer = 0;
            } else {
                // Berjalan maju
        ...
        // (Logika ini tetap sama, dipicu oleh main.js)
